import type { AppTranslations, LocaleKey } from './index';

export const appTranslations: Record<LocaleKey, AppTranslations> = {
  en: {
    app: {
      name: 'MailAgent',
      tagline: 'AI-native workspace for mission critical email',
    },
    nav: {
      product: 'Product',
      features: 'Features',
      automation: 'Automation',
      resources: 'Resources',
      login: 'Sign in',
      register: 'Create account',
      home: 'Home',
      mail: 'Mail',
      calendar: 'Calendar',
      contacts: 'Contacts',
      tasks: 'Tasks',
      analytics: 'Analytics',
      ai: 'AI Assistant',
      settings: 'Settings',
      profile: 'Profile',
      logout: 'Logout',
      account: 'Account',
      notifications: 'Notifications',
      language: 'Language',
      lightMode: 'Light Mode',
      darkMode: 'Dark Mode',
      searchPlaceholder: 'Search emails, contacts, events...',
    },
    common: {
      loading: 'Loading...',
      getStarted: 'Get started',
      contactSales: 'Contact sales',
      signIn: 'Sign in',
      createAccount: 'Create account',
      backToLogin: 'Back to login',
      continue: 'Continue',
      cancel: 'Cancel',
      send: 'Send',
      compose: 'Compose',
      refresh: 'Refresh',
      refreshing: 'Refreshing...',
      saving: 'Saving...',
      sending: 'Sending...',
      otpPlaceholder: '000000',
      emailPlaceholder: 'you@company.com',
      passwordPlaceholder: 'Enter your password',
      confirmPasswordPlaceholder: 'Repeat your password',
      searchPlaceholder: 'Search emails, people, or labels...',
      genericError: 'Something went wrong. Please try again.',
      passwordMismatch: 'Passwords do not match.',
      invalidToken: 'Invalid or expired reset token.',
      search: 'Search',
      save: 'Save',
      delete: 'Delete',
      edit: 'Edit',
      close: 'Close',
      confirm: 'Confirm',
      yes: 'Yes',
      no: 'No',
      discard: 'Discard',
      viewAll: 'View All',
      join: 'Join',
      retry: 'Retry',
      user: 'User',
      version: 'Version 1.0.0',
      allCaughtUp: "All caught up!",
      noSubject: 'No subject',
      unnamedContact: 'Unnamed contact',
      toggleTheme: 'Toggle theme',

    },
    timeAgo: {
      justNow: 'Just now',
      minutesAgo: '{minutes}m ago',
      hoursAgo: '{hours}h ago',
      daysAgo: '{days}d ago',
    },
    date: {
      today: 'Today',
      yesterday: 'Yesterday',
      daysAgo: '{days} days ago',
      never: 'Never',
    },
    greetings: {
      morning: 'Good Morning',
      afternoon: 'Good Afternoon',
      evening: 'Good Evening',
      subtitle: "Here's what's happening today",
    },
    landing: {
      hero: {
        eyebrow: 'AI-native communications layer',
        title: 'Orchestrate every inbox with confidence',
        subtitle:
          'MailAgent blends intelligent triage, automated drafting, and voice-ready copilots so every response is on brand and on time.',
        primaryCta: 'Book a live demo',
        secondaryCta: 'See the platform',
      },
      highlights: [
        { label: 'Faster responses', value: '2.4×' },
        { label: 'Manual triage reduced', value: '-63%' },
        { label: 'Playbooks automated', value: '180+' },
      ],
      features: [
        {
          title: 'Unified material inbox',
          description: 'Blend Gmail, Outlook, and IMAP accounts into a single material design workspace.',
          bullets: [
            'Adaptive theming for light & dark environments',
            'Enterprise labels, filters, and retention controls',
            'Context-rich profile cards for every sender',
          ],
        },
        {
          title: 'AI drafting that sounds like you',
          description: 'Guided prompts, tone controls, and approval flows keep humans in the loop.',
          bullets: [
            'Reusable templates with live variables',
            'Voice-to-email and audio playback',
            'Compliance friendly audit history',
          ],
        },
        {
          title: 'Operations grade analytics',
          description: 'Reveal SLA risk, queue volume, and automation impact in real time.',
          bullets: [
            'Smart alerts for VIP accounts',
            'Monte Carlo forecasting for queues',
            'Embedded dashboards for every team lead',
          ],
        },
      ],
      modules: [
        {
          eyebrow: 'Intelligent triage',
          title: 'Score every thread automatically',
          description:
            'Custom models learn what matters to your teams, then route email to AI or humans with confidence levels.',
          items: ['Sentiment & urgency detection', 'Queue balancing automations', 'Playbook suggestions'],
        },
        {
          eyebrow: 'Connected ecosystem',
          title: 'Built for enterprise reality',
          description:
            'Deep integrations with Google Workspace, Microsoft 365, Exchange, and any IMAP/SMTP or CalDAV source.',
          items: ['SOC 2 & ISO controls', 'Granular role-based access', 'Audit-ready logging'],
        },
      ],
      testimonial: {
        quote:
          'MailAgent let our 70-person support org operate like 200. The AI assistant drafts perfect replies, while the dashboard shows exactly where human focus is needed.',
        author: 'Chiara Leone',
        role: 'Head of Service Operations, Aeris',
      },
      secondaryCta: {
        title: 'Ready for a material refresh of your email ops?',
        subtitle: 'Bring every workflow into a single AI-native workspace with MailAgent.',
        primaryCta: 'Talk to product',
        secondaryCta: 'Download one-pager',
      },
    },
    auth: {
      login: {
        title: 'Welcome back',
        subtitle: 'Secure authentication with MFA and session guardrails.',
        form: {
          emailLabel: 'Work email',
          passwordLabel: 'Password',
          submit: 'Sign in',
          cta: 'Need an account?',
        },
        links: {
          primary: { href: '/auth/register', label: 'Create an account' },
          secondary: { href: '/auth/forgot-password', label: 'Forgot password?' },
        },
      },
      otp: {
        title: 'Verify the one-time code',
        subtitle: 'Check your inbox for a 6-digit security code.',
        form: {
          otpLabel: 'One-time code',
          submit: 'Verify & continue',
        },
        links: {
          primary: { href: '/auth/login', label: 'Back to login' },
        },
      },
      register: {
        title: 'Create your workspace',
        subtitle: 'Provision your first inbox in less than two minutes.',
        form: {
          firstNameLabel: 'First name',
          firstNamePlaceholder: 'Alex',
          lastNameLabel: 'Last name',
          lastNamePlaceholder: 'Smith',
          emailLabel: 'Work email',
          passwordLabel: 'Password',
          confirmPasswordLabel: 'Confirm password',
          submit: 'Create account',
          agreeToTerms: 'I agree to the ',
          termsOfService: 'Terms of Service',
          and: ' and ',
          privacyPolicy: 'Privacy Policy',
          termsRequired: 'You must agree to the Terms of Service and Privacy Policy to register.',
        },
        links: {
          primary: { href: '/auth/login', label: 'Already have an account? Sign in' },
        },
      },
      forgotPassword: {
        title: 'Reset your password',
        subtitle: 'We will send a secure link to your email address.',
        form: {
          emailLabel: 'Work email',
          submit: 'Send reset link',
        },
        success: {
          title: 'Check your inbox',
          description: 'If an account exists we just sent you a secure link.',
          actionLabel: 'Back to login',
        },
      },
      resetPassword: {
        title: 'Choose a new password',
        subtitle: 'Your password must be at least 12 characters.',
        form: {
          passwordLabel: 'New password',
          confirmPasswordLabel: 'Confirm password',
          submit: 'Update password',
        },
        success: {
          title: 'Password updated',
          description: 'You can now sign in with your new credentials.',
          actionLabel: 'Go to login',
        },
      },
    },
    dashboard: {
      index: {
        loading: 'Loading...',
        redirecting: 'Redirecting to email...',
      },
      home: {
        stats: {
          unreadEmails: 'Unread Emails',
          todayEvents: "Today's Events",
          pendingTasks: 'Pending Tasks',
          contacts: 'Contacts',
        },
        sections: {
          upcomingEvents: 'Upcoming Events',
          upcomingEventsSubtitle: 'Your schedule for today',
          noEvents: 'No events scheduled for today',
          viewCalendar: 'View Calendar',
          priorityInbox: 'Priority Inbox',
          priorityInboxSubtitle: 'Starred and important emails',
          noStarredEmails: 'No starred emails',
          viewAllEmails: 'View All Emails',
          aiInsights: 'AI Insights',
          aiInsightsSubtitle: 'Smart suggestions for you',
          smartReplyReady: 'Smart Reply Ready',
          aiAssistMessage: 'You have {count} unread email{plural} that can be managed with AI assistance.',
          aiAssistMessageSingular: 'You have {count} unread email that can be managed with AI assistance.',
          viewSuggestions: 'View Suggestions',
          inboxStatus: 'Inbox Status',
          inboxStatusMessage: '{count} unread - Keep going!',
          quickActions: 'Quick Actions',
          analyzeInbox: 'Analyze Inbox',
          smartCompose: 'Smart Compose',
          recentConnections: 'Recent Connections',
          recentConnectionsSubtitle: 'Latest contacts',
          noRecentContacts: 'No recent contacts',
        },
      },
      email: {
        title: 'Inbox control center',
        descriptionTemplate: '{count} unread conversations — stay focused on the work that matters.',
        searchPlaceholder: 'Search threads, senders, or labels',
        noProviders: 'Connect a mail provider to send and receive email.',
        selectEmail: 'Select a conversation to start collaborating.',
        allAccountsLabel: 'All accounts',
        starredFolderLabel: 'Starred',
        quickFilters: {
          title: 'Quick Filters',
          unread: 'Unread',
          today: 'Today',
          thisWeek: 'This Week',
          hasAttachments: 'Has Attachments',
          important: 'Important',
        },
        bulkBar: {
          selected: '{count} selected',
          markUnread: 'Mark unread',
          markRead: 'Mark read',
          delete: 'Delete',
          archive: 'Archive',
          selectAll: 'Select all',
          clear: 'Clear selection',
          moreActions: 'More actions',
          addStar: 'Add Star',
          removeStar: 'Remove Star',
          addLabels: 'Add Labels',
          moveToFolder: 'Move to Folder',
        },
        messages: {
          loadMoreFailed: 'Failed to load more emails',
          markRead: 'Marked {count} email(s) as read',
          markReadFailed: 'Failed to mark emails as read',
          markUnread: 'Marked {count} email(s) as unread',
          markUnreadFailed: 'Failed to mark emails as unread',
          deleteConfirm: 'Are you sure you want to delete {count} email(s)?',
          deleted: 'Deleted {count} email(s)',
          deleteFailed: 'Failed to delete emails',
          starred: 'Starred {count} email(s)',
          starFailed: 'Failed to star emails',
          unstarred: 'Unstarred {count} email(s)',
          unstarFailed: 'Failed to unstar emails',
          labelsAdded: 'Added labels to {count} email(s)',
          labelsFailed: 'Failed to add labels',
          moved: 'Moved {count} email(s) to {folder}',
          moveFailed: 'Failed to move emails',
          loadLabelsFailed: 'Failed to load labels',
          selectedCount: '{count} selected',
          move: 'Move',
          emailSent: 'Email sent successfully ✓',
        },
        composer: {
          title: 'New Message',
          titleReply: 'Reply',
          titleForward: 'Forward',
          from: 'From',
          to: 'To',
          toLabel: 'To:',
          toPlaceholder: 'recipient@example.com',
          cc: 'Cc',
          ccLabel: 'Cc:',
          bcc: 'Bcc',
          bccLabel: 'Bcc:',
          subject: 'Subject',
          subjectLabel: 'Subject:',
          subjectPlaceholder: 'Email subject',
          bodyLabel: 'Message',
          bodyPlaceholder: 'Compose your message...',
          attachFiles: 'Attach files',
          removeAttachment: 'Remove attachment',
          attachments: 'Attachments ({count})',
          formatting: 'Formatting',
          undo: 'Undo',
          redo: 'Redo',
          heading1: 'Heading 1',
          heading2: 'Heading 2',
          heading3: 'Heading 3',
          bold: 'Bold',
          italic: 'Italic',
          underline: 'Underline',
          strikethrough: 'Strikethrough',
          bulletList: 'Bullet list',
          numberedList: 'Numbered list',
          quote: 'Quote',
          link: 'Insert link',
          send: 'Send',
          saveDraft: 'Save draft',
          discard: 'Discard',
          sending: 'Sending...',
          saving: 'Saving...',
          savedJustNow: 'Saved just now',
          savedMinutesAgo: 'Saved {minutes}m ago',
          savedHoursAgo: 'Saved {hours}h ago',
          selectProvider: 'Select email account',
          noProvider: 'No email provider selected',
          validationTo: 'Please enter at least one recipient',
          validationSubject: 'Please enter a subject',
          sendError: 'Failed to send email. Please try again.',
          uploadError: 'Failed to upload attachments',
          fileTooLarge: 'File {filename} is too large (max 10MB)',
          draftSaved: 'Draft saved successfully',
          discardConfirm: 'Discard unsaved changes?',
        },
        advancedSearch: {
          title: 'Advanced Search',
          active: 'active',
          searchInSubjectBody: 'Search in subject and body',
          enterKeywords: 'Enter keywords...',
          fromSender: 'From (Sender)',
          senderPlaceholder: 'sender@example.com',
          filterBySender: 'Filter by sender email address',
          dateRange: 'Date Range',
          fromDate: 'From Date',
          toDate: 'To Date',
          statusFilters: 'Status Filters',
          hasAttachments: 'Has attachments',
          starredOnly: 'Starred only',
          readStatus: 'Read Status',
          unreadOnly: 'Unread only',
          readOnly: 'Read only',
          resetAll: 'Reset All',
          cancel: 'Cancel',
          search: 'Search',
        },
        attachment: 'Attachment',
      },
      labels: {
        title: 'Labels',
        manageLabels: 'Manage Labels',
        addLabel: 'Add Label',
        createLabel: 'Create Label',
        editLabel: 'Edit Label',
        labelName: 'Label Name',
        labelNameRequired: 'Label name is required',
        chooseColor: 'Choose a color',
        createButton: 'Create',
        updateButton: 'Update',
        cancelButton: 'Cancel',
        deleteConfirm: 'Are you sure you want to delete this label?',
        failedToCreate: 'Failed to create label',
        failedToUpdate: 'Failed to update label',
        failedToDelete: 'Failed to delete label',
        noLabels: 'No labels yet',
        noLabelsDescription: 'Create labels to organize your emails',
        closeButton: 'Close',
        manageLabelsTooltip: 'Manage Labels',
        addRemoveLabels: 'Add or remove labels',
        noLabelsAvailable: 'No labels available',
        addLabelText: 'Add label',
      },
      conversations: {
        title: 'Conversations',
        noConversations: 'No conversations found',
        noConversationsDescription: 'Try adjusting your filters or start a new email',
        failedToLoad: 'Failed to load conversations',
        loadMore: 'Load More',
        loading: 'Loading...',
        messages: 'messages',
        participants: 'participants',
        yesterday: 'Yesterday',
        retry: 'Retry',
      },
      emailList: {
        loading: 'Loading emails...',
        empty: 'No emails match your filters.',
        threads: { singular: 'thread', plural: 'threads' },
        inboxLabel: 'Inbox',
        advancedSearch: 'Advanced Search',
        emailListView: 'Email List',
        conversationView: 'Conversation View',
        openSidebar: 'Open sidebar',
        markAsRead: 'Mark as read',
        markAsUnread: 'Mark as unread',
        archive: 'Archive',
      },
      emailView: {
        folderLabel: 'Folder',
        attachments: 'Attachments',
        to: 'To',
        cc: 'Cc',
        labels: 'Labels',
        date: 'Date',
        selectEmail: 'Select an email to preview the full thread.',
        back: 'Back',
        archive: 'Archive',
        more: 'More',
        download: 'Download',
        reply: 'Reply',
        replyAll: 'Reply All',
        forward: 'Forward',
        summaryTitle: 'AI summary',
        summaryGenerate: 'Generate summary',
        summaryRegenerate: 'Refresh summary',
        summaryShow: 'Show Summary',
        summaryHide: 'Hide Summary',
        summaryEmpty: 'Ask the Copilot to explain this conversation.',
        categoriesGenerate: 'Categorize with AI',
        categoriesLoaded: 'Categories loaded',
        categoriesLoading: 'Categorizing...',
        smartRepliesTitle: 'Smart replies',
        smartRepliesGenerate: 'Generate smart replies',
        smartRepliesRegenerate: 'Refresh replies',
        smartRepliesLoaded: 'Smart replies loaded',
        smartRepliesLoading: 'Fetching smart replies...',
        smartRepliesEmpty: 'No smart replies available yet.',
        labelTitle: 'Suggested labels',
        labelEmpty: 'AI suggested labels will appear here.',
        memoryTitle: 'Workspace memory',
        memoryDescription: 'Search the RAG layer for similar conversations, docs, and prior decisions.',
        memoryPlaceholder: 'Add keywords to refine the search (optional)',
        memoryGenerate: 'Search memory',
        memoryRegenerate: 'Refresh memory',
        memoryLoading: 'Pulling relevant memories...',
        memoryEmpty: 'No relevant memories yet. Try refining the search.',
        memoryCopy: 'Copy',
        memoryCopied: 'Copied',
        memoryUse: 'Use in reply',
        memoryQueryRequired: 'Enter a search query or open an email to search.',
        memorySourceEmail: 'Email',
        memorySourceDocument: 'Document',
        memoryConfidenceLabel: 'Match strength',
        memoryConfidenceHigh: 'High',
        memoryConfidenceMedium: 'Medium',
        memoryConfidenceLow: 'Light',
        memoryUnknownSender: 'Unknown sender',
        memoryLastQueryPrefix: 'Using query:',
        memoryCopyError: 'Unable to copy this memory right now.',
        memoryError: 'Unable to search memory right now.',
      },
      calendar: {
        title: 'Calendar',
        description: 'Manage your calendar events',
        refreshTooltip: 'Refresh calendars',
        allCalendars: 'All Calendars',
        clearSelection: 'Hide all',
        filterTitle: 'Calendars',
        noProviders: 'No calendar providers configured',
        noProvidersDescription: 'Please configure a calendar provider in the Providers page',
        loading: 'Loading calendar...',
        createEvent: 'New Event',
        editEvent: 'Edit Event',
        deleteEvent: 'Delete Event',
        deleteConfirm: 'Are you sure you want to delete this event?',
        eventDetails: 'Event Details',
        startTime: 'Start Time',
        endTime: 'End Time',
        location: 'Location',
        eventDescription: 'Description',
        attendees: 'Attendees',
        saveEvent: 'Save Event',
        cancelEvent: 'Cancel',
        quickEventPlaceholder: 'Schedule a team sync for Friday morning...',
        today: 'Today',
        day: 'Day',
        week: 'Week',
        month: 'Month',
        eventTitle: 'Event Title',
        eventTitleLabel: 'Event Title',
        startDateTime: 'Start Date/Time',
        endDateTime: 'End Date/Time',
        calendarLabel: 'Calendar',
        noProviderAlert: 'No calendar provider available. Please connect a calendar provider first.',
        createEventDialogTitle: 'Create New Event',
        aiInsightsTitle: 'AI Insights',
        aiOptimizeMessage: 'You have {count} events scheduled. Would you like AI to optimize your calendar?',
        openAiAssistant: 'Open AI Assistant',
        noEventsMessage: 'No events scheduled. Click on the calendar to create your first event!',
        failedToCreateEvent: 'Failed to create event. Please try again.',
        failedToCreateQuickEvent: 'Failed to create quick event. Please try again.',
      },
      contacts: {
        title: 'Contacts',
        description: 'Unified address book for every connected provider.',
        searchPlaceholder: 'Search contacts...',
        companyPlaceholder: 'Company',
        providerFilterLabel: 'Provider',
        allProvidersLabel: 'All providers',
        applyFilters: 'Apply filters',
        clearFilters: 'Clear filters',
        refresh: 'Refresh',
        sync: 'Sync provider',
        syncing: 'Syncing...',
        syncDisabledTooltip: 'Select a provider to run a sync',
        new: 'New',
        noContactsFound: 'No contacts found',
        selectContactMessage: 'Select a contact to view details',
        actions: {
          email: 'Email',
          schedule: 'Schedule',
          addNote: 'Add Note',
          aiInsights: 'AI Insights',
        },
        details: {
          contactInformation: 'Contact Information',
          email: 'Email',
          phone: 'Phone',
          company: 'Company',
          location: 'Location',
          lastInteraction: 'Last Interaction',
          notAvailable: 'Not available',
        },
        tabs: {
          details: 'Details',
          activity: 'Activity',
          notes: 'Notes',
        },
        activity: {
          title: 'Recent Activity',
          noActivity: 'No recent activity',
        },
        notes: {
          title: 'Notes',
          noNotes: 'No notes yet',
          addNote: 'Add Note',
        },
        list: {
          emptyTitle: 'No contacts yet',
          emptyDescription: 'Connect a contacts-capable provider or adjust your filters.',
          loading: 'Loading contacts...',
          showing: 'Showing {count} of {total} contacts',
          tableHeaders: {
            name: 'Name',
            email: 'Email',
            phone: 'Phone',
            company: 'Company',
            provider: 'Provider',
            lastSynced: 'Last synced',
            actions: 'Actions',
          },
        },
        alerts: {
          loadError: 'Unable to load contacts right now. Please try again.',
          syncSuccess: 'Contacts sync completed successfully.',
          syncError: 'Unable to sync contacts. Please try again.',
          deleteConfirmTitle: 'Delete contact',
          deleteConfirmDescription: 'Are you sure you want to delete this contact?',
          deleteSuccess: 'Contact deleted successfully.',
          deleteError: 'Unable to delete this contact. Please try again.',
          selectProvider: 'Please select a provider',
          failedToSave: 'Failed to save contact. Please try again.',
        },
        createDialog: {
          title: 'New Contact',
          provider: 'Provider',
          name: 'Name',
          email: 'Email',
          company: 'Company',
          jobTitle: 'Job Title',
          notes: 'Notes',
          create: 'Create Contact',
          cancel: 'Cancel',
        },
        editDialog: {
          title: 'Edit Contact',
          description: 'Update basic information for this contact.',
          firstName: 'First name',
          lastName: 'Last name',
          displayName: 'Display name',
          name: 'Name',
          email: 'Email',
          phone: 'Primary phone',
          company: 'Company',
          jobTitle: 'Job Title',
          notes: 'Notes',
          save: 'Save Changes',
          cancel: 'Cancel',
          success: 'Contact updated successfully.',
          error: 'Unable to update this contact. Please try again.',
        },
      },
      tasks: {
        title: 'Tasks',
        workspace: 'Workspace',
        allTasks: 'All Tasks',
        today: 'Today',
        upcoming: 'Upcoming',
        myProjects: 'My Projects',
        addNewList: 'Add New List',
        filter: 'Filter',
        newTask: 'New Task',
        addTaskPlaceholder: 'Add a task for Today, press Enter to save',
        taskDetails: 'Task Details',
      },
      providers: {
        title: 'Providers & Integrations',
        descriptionTemplate: 'Connect mailboxes, calendars, and synced sources for {email}',
        refresh: 'Refresh',
        refreshing: 'Refreshing...',
        connectedTab: 'Connected Providers',
        addTab: 'Add Provider',
        connectedTitle: 'Connected providers',
        connectedDescription: 'Manage linked inboxes, calendars, and automation sources for your tenant.',
        loading: 'Loading providers...',
        googleTitle: 'Google',
        googleDescription: 'Connect Gmail and Google Calendar',
        googleConnect: 'Connect Google Account',
        googleConnecting: 'Connecting...',
        googleFeatures: {
          description: 'Connect your Google account to sync emails and calendar events.',
          feature1: 'Read and send emails via Gmail',
          feature2: 'Access Google Calendar events',
          feature3: 'Sync contacts (optional)',
        },
        microsoftTitle: 'Microsoft',
        microsoftDescription: 'Connect Outlook and Microsoft Calendar',
        microsoftConnect: 'Connect Microsoft Account',
        microsoftConnecting: 'Connecting...',
        microsoftFeatures: {
          description: 'Connect your Microsoft account to access emails and calendar.',
          feature1: 'Read and send emails via Outlook',
          feature2: 'Access Microsoft Calendar events',
          feature3: 'Sync contacts (optional)',
        },
        genericTitle: 'Generic IMAP/CalDAV',
        genericDescription: 'Connect any IMAP/SMTP or CalDAV provider',
        genericConnect: 'Configure Generic Provider',
        genericFeatures: {
          description: 'Connect any IMAP/SMTP or CalDAV provider to unify email and calendar data.',
          feature1: 'IMAP/SMTP email access',
          feature2: 'CalDAV calendar sync',
          feature3: 'CardDAV contacts sync',
        },
        genericCard: {
          title: 'IMAP & CalDAV',
          description: 'Connect custom IMAP/SMTP and CalDAV providers',
          subtitle: 'Bring any compatible mailbox or calendar by entering your server credentials.',
          feature1: 'IMAP/SMTP support for inbound/outbound email',
          feature2: 'Optional CalDAV sync for calendars',
          feature3: 'Perfect for Fastmail, Yahoo, iCloud, and custom servers',
          configureButton: 'Configure IMAP / CalDAV',
          dialogTitle: 'Connect IMAP/SMTP & CalDAV Provider',
          dialogSubtitle: 'Configure your email and calendar server settings',
          basicInfo: 'Basic Information',
          emailAddress: 'Email Address',
          displayName: 'Display Name',
          imapSettings: 'IMAP Settings (Incoming Mail)',
          imapHost: 'IMAP Host',
          imapHostPlaceholder: 'imap.example.com',
          port: 'Port',
          username: 'Username',
          password: 'Password',
          smtpSettings: 'SMTP Settings (Outgoing Mail)',
          smtpSubtitle: 'Leave empty to use IMAP credentials',
          smtpHost: 'SMTP Host',
          smtpHostPlaceholder: 'smtp.example.com',
          caldavSettings: 'CalDAV Settings (Calendar Sync)',
          enableCalendar: 'Enable Calendar',
          caldavUrl: 'CalDAV URL',
          caldavUrlPlaceholder: 'https://caldav.example.com/calendars/user',
          cancel: 'Cancel',
          connecting: 'Connecting...',
          connectProvider: 'Connect Provider',
        },
        helpTitle: 'Need Help?',
        helpGoogle: 'Click "Connect Google Account" and sign in with your Google credentials. Grant access to Gmail, Calendar, and Contacts.',
        helpMicrosoft: 'Use "Connect Microsoft Account" and sign in with your Outlook/Microsoft 365 credentials.',
        helpGeneric: 'Provide IMAP/SMTP server details from your provider. CalDAV enables calendar sync.',
        commonSettingsTitle: 'Common IMAP/SMTP Settings:',
        successMessage: 'Provider connected successfully!',
        deleteSuccess: 'Provider disconnected successfully!',
        deleteConfirm: 'Are you sure you want to disconnect {email}?',
        disconnecting: 'Disconnecting...',
        disconnect: 'Disconnect',
        oauthError: 'OAuth error:',
        addProvider: 'Add a provider',
        noProvidersMessage: 'No providers connected yet. Connect a provider to get started.',
        badges: {
          default: 'Default',
          active: 'Active',
          inactive: 'Inactive',
          email: 'Email',
          calendar: 'Calendar',
          contacts: 'Contacts',
        },
        lastSynced: 'Last synced: {date}',
        neverSynced: 'Never',
      },
      settings: {
        title: 'Workspace Settings',
        description: 'Update your personal profile, manage connected providers, and handle account actions.',
        preferences: 'Preferences',
        profileTitle: 'Profile Settings',
        profileDescription: 'Keep your personal information up to date.',
        firstName: 'First Name',
        lastName: 'Last Name',
        emailReadonly: 'Email (read-only)',
        saving: 'Saving...',
        saveChanges: 'Save changes',
        successMessage: 'Settings updated successfully',
        providersTitle: 'Email & Calendar Providers',
        providersDescription: 'Manage the accounts powering your inbox and calendar sync.',
        manageProviders: 'Manage Providers',
        dangerZoneTitle: 'Danger Zone',
        dangerZoneDescription: 'Delete your account and all associated data permanently.',
        dangerZoneWarning: 'This action cannot be undone. Please make sure you have exported any important information before proceeding.',
        deleteAccount: 'Delete account',
        deleting: 'Deleting...',
        deleteConfirm: 'Are you sure you want to delete your account? This action cannot be undone.',
        sidebarTitle: 'Settings',
        sidebarSubtitle: 'Manage your preferences',
        sections: {
          general: 'General',
          ai: 'AI Agent',
          accounts: 'Mail Accounts',
          account: 'Account',
          notifications: 'Notifications',
        },
        generalPanel: {
          title: 'General Settings',
          description: 'Configure appearance, language, and notification defaults.',
          appearanceTitle: 'Appearance',
          themeLabel: 'Theme',
          themeHint: 'Choose "System" to match your operating system automatically.',
          themeOptions: {
            light: 'Light',
            dark: 'Dark',
            system: 'System',
          },
          languageTitle: 'Language & Region',
          languageLabel: 'Language',
          languageHint: 'This only affects the MailAgent interface, not email content.',
          languageOptions: {
            en: 'English (US)',
            it: 'Italiano',
            es: 'Español',
            fr: 'Français',
            de: 'Deutsch',
          },
          timezoneLabel: 'Timezone',
          timezoneOptions: {
            pst: '(GMT-08:00) Pacific Time',
            est: '(GMT-05:00) Eastern Time',
            gmt: '(GMT+00:00) London',
            cet: '(GMT+01:00) Rome',
            jst: '(GMT+09:00) Tokyo',
          },
          notificationsTitle: 'Notifications',
          notificationsDescription: 'Fine-tune how you receive alerts.',
          notificationOptions: {
            newEmails: 'New Emails',
            newEmailsDescription: 'Get notified when you receive new emails',
            calendarReminders: 'Calendar Reminders',
            calendarRemindersDescription: 'Reminders for upcoming events',
            taskDeadlines: 'Task Deadlines',
            taskDeadlinesDescription: 'Alerts when tasks are due',
          },
          reset: 'Reset',
          save: 'Save changes',
        },
        mailAccountsPanel: {
          title: 'Mail Accounts',
          description: 'Review the accounts powering your inbox, calendar, and contacts sync.',
          empty: 'No accounts connected yet. Use the actions below to add one.',
          manage: 'Manage providers',
          add: 'Add a provider',
          refresh: 'Refresh',
          defaultBadge: 'Default',
          emailBadge: 'Email',
          calendarBadge: 'Calendar',
          contactsBadge: 'Contacts',
          lastSyncPrefix: 'Last sync:',
          neverSynced: 'Never synced',
          connectedProviders: 'Connected providers',
          connectedProvidersSubtitle: 'Manage linked inboxes, calendars, and automation sources for your tenant.',
          providerTypes: {
            google: 'Google Workspace',
            microsoft: 'Microsoft 365',
            generic: 'Generic IMAP/SMTP',
          },
          loadError: 'Failed to load providers. Please try again.',
          connectSuccess: 'Provider connected successfully!',
          connectRedirect: 'Redirecting to provider. Complete the authorization flow to finish connecting.',
          disconnectSuccess: 'Provider disconnected successfully!',
          saveSuccess: 'Settings saved successfully',
          saveError: 'Failed to save settings',
          helpTitle: 'Need Help?',
          helpGoogle: 'Google: Click "Connect Google Account" and sign in with your Google credentials. Make sure you have access to Gmail and Google Calendar.',
          helpMicrosoft: 'Microsoft: Use "Connect Microsoft Account" and sign in with your Microsoft 365 or Outlook.com credentials.',
          helpIMAP: 'Generic IMAP: Provide IMAP/SMTP server details, your email address, and password or app-specific password.',
          helpCommonSettings: 'Common IMAP/SMTP Settings:',
          helpGmail: 'Gmail: imap.gmail.com:993, smtp.gmail.com:587 (requires app password)',
          helpOutlook: 'Outlook.com: outlook.office365.com:993, smtp.office365.com:587',
          helpYahoo: 'Yahoo: imap.mail.yahoo.com:993, smtp.mail.yahoo.com:587',
        },
        aiPanel: {
          title: 'AI Agent Settings',
          description: 'Decide how the copilot assists you across the workspace.',
          smartReplies: 'Smart replies',
          smartRepliesDescription: 'AI suggestions for quick responses.',
          summarization: 'Email summarization',
          summarizationDescription: 'Automatic context for long threads.',
          scheduling: 'Smart scheduling',
          schedulingDescription: 'Suggest meeting times automatically.',
          save: 'Save changes',
        },
        accountPanel: {
          title: 'Account Settings',
          description: 'Manage your profile information.',
          profileInformation: 'Profile information',
          fullName: 'Full Name',
          email: 'Email',
          role: 'Role',
          readOnlyNotice: 'Account details are managed centrally. Contact your administrator to request changes.',
          dangerTitle: 'Danger Zone',
          dangerDescription: 'Delete your account and all associated data permanently.',
          deleteCta: 'Delete account',
        },
        notificationsPanel: {
          title: 'Notification Settings',
          description: 'Control how you receive alerts.',
          emailToggle: 'Email notifications',
        },
      },
      folders: {
        all: 'All Mail',
        inbox: 'Inbox',
        sent: 'Sent',
        drafts: 'Drafts',
        trash: 'Trash',
        spam: 'Spam',
        archive: 'Archive',
        outbox: 'Outbox',
        starred: 'Starred',
      },
    },
    cookies: {
      title: 'Cookie Settings',
      description: 'We use cookies to enhance your browsing experience, serve personalized content, and analyze our traffic. By clicking "Accept All", you consent to our use of cookies.',
      privacyLink: 'Privacy Policy',
      customize: 'Customize',
      showDetails: 'Show details',
      hideDetails: 'Hide details',
      necessary: {
        title: 'Necessary Cookies',
        description: 'Essential for the website to function properly. These cannot be disabled.',
      },
      analytics: {
        title: 'Analytics Cookies',
        description: 'Help us understand how visitors interact with our website.',
      },
      marketing: {
        title: 'Marketing Cookies',
        description: 'Used to deliver personalized advertisements.',
      },
      acceptAll: 'Accept All',
      rejectAll: 'Reject All',
      savePreferences: 'Save Preferences',
    },
    errors: {
      notFound: {
        title: '404',
        subtitle: 'Page Not Found',
        description: 'Sorry, we couldn\'t find the page you\'re looking for.',
        action: 'Go Back Home',
      },
      serverError: {
        title: '500',
        subtitle: 'Server Error',
        description: 'Something went wrong on our end. Please try again later.',
        action: 'Go Back Home',
      },
    },
  },
  it: {
    app: {
      name: 'MailAgent',
      tagline: 'Workspace AI per le email mission critical',
    },
    nav: {
      product: 'Prodotto',
      features: 'Funzionalità',
      automation: 'Automazioni',
      resources: 'Risorse',
      login: 'Accedi',
      register: 'Crea account',
      home: 'Home',
      mail: 'Posta',
      calendar: 'Calendario',
      contacts: 'Contatti',
      tasks: 'Attività',
      analytics: 'Analisi',
      ai: 'Assistente AI',
      settings: 'Impostazioni',
      profile: 'Profilo',
      logout: 'Esci',
      account: 'Account',
      notifications: 'Notifiche',
      language: 'Lingua',
      lightMode: 'Modalità Chiara',
      darkMode: 'Modalità Scura',
      searchPlaceholder: 'Cerca email, contatti, eventi...',
    },
    common: {
      loading: 'Caricamento...',
      getStarted: 'Inizia ora',
      contactSales: 'Contatta il team',
      signIn: 'Accedi',
      createAccount: 'Crea account',
      backToLogin: 'Torna al login',
      continue: 'Continua',
      cancel: 'Annulla',
      send: 'Invia',
      compose: 'Scrivi',
      refresh: 'Aggiorna',
      refreshing: 'Aggiornamento...',
      saving: 'Salvataggio...',
      sending: 'Invio...',
      otpPlaceholder: '000000',
      emailPlaceholder: 'nome@azienda.com',
      passwordPlaceholder: 'Inserisci la password',
      confirmPasswordPlaceholder: 'Ripeti la password',
      searchPlaceholder: 'Cerca email, persone o etichette...',
      genericError: 'Qualcosa è andato storto. Riprova.',
      passwordMismatch: 'Le password non coincidono.',
      invalidToken: 'Token di ripristino non valido o scaduto.',
      search: 'Cerca',
      save: 'Salva',
      delete: 'Elimina',
      edit: 'Modifica',
      close: 'Chiudi',
      confirm: 'Conferma',
      yes: 'Sì',
      no: 'No',
      discard: 'Scarta',
      viewAll: 'Vedi Tutti',
      join: 'Partecipa',
      retry: 'Riprova',
      user: 'Utente',
      version: 'Versione 1.0.0',
      allCaughtUp: "Tutto a posto!",
      noSubject: 'Nessun oggetto',
      unnamedContact: 'Contatto senza nome',
      toggleTheme: 'Cambia tema',
    },
    timeAgo: {
      justNow: 'Proprio ora',
      minutesAgo: '{minutes}m fa',
      hoursAgo: '{hours}h fa',
      daysAgo: '{days}g fa',
    },
    date: {
      today: 'Oggi',
      yesterday: 'Ieri',
      daysAgo: '{days} giorni fa',
      never: 'Mai',
    },
    greetings: {
      morning: 'Buongiorno',
      afternoon: 'Buon pomeriggio',
      evening: 'Buonasera',
      subtitle: "Ecco cosa succede oggi",
    },
    landing: {
      hero: {
        eyebrow: 'Piattaforma AI per le comunicazioni',
        title: 'Controlla ogni inbox con serenità',
        subtitle:
          'MailAgent unisce triage intelligente, bozze automatiche e assistenti vocali per risposte puntuali e coerenti.',
        primaryCta: 'Richiedi una demo',
        secondaryCta: 'Scopri il prodotto',
      },
      highlights: [
        { label: 'Risposte più rapide', value: '2,4×' },
        { label: 'Triage manuale ridotto', value: '-63%' },
        { label: 'Playbook automatizzati', value: '180+' },
      ],
      features: [
        {
          title: 'Inbox unificata',
          description: 'Integra Gmail, Outlook e ogni account IMAP in un’unica interfaccia Material.',
          bullets: [
            'Tema adattivo chiaro/scuro',
            'Etichette e filtri enterprise',
            'Schede profilo con contesto completo',
          ],
        },
        {
          title: 'Bozze AI personalizzate',
          description: 'Prompt guidati, controllo del tono e flussi di approvazione.',
          bullets: [
            'Template riutilizzabili con variabili',
            'Dettatura vocale e lettura audio',
            'Storico audit conforme',
          ],
        },
        {
          title: 'Analisi operative',
          description: 'Metriche SLA, volumi di coda e impatto delle automazioni in tempo reale.',
          bullets: [
            'Avvisi smart per account VIP',
            'Previsioni sulle code',
            'Dashboard per ogni team leader',
          ],
        },
      ],
      modules: [
        {
          eyebrow: 'Triage intelligente',
          title: 'Valuta ogni conversazione automaticamente',
          description:
            'Modelli personalizzati apprendono le tue priorità e indirizzano il thread verso AI o persone.',
          items: [
            'Rilevazione di sentiment e urgenza',
            'Bilanciamento automatico delle code',
            'Suggerimenti di playbook',
          ],
        },
        {
          eyebrow: 'Ecosistema connesso',
          title: 'Pronto per l’impresa',
          description:
            'Integrazioni profonde con Google Workspace, Microsoft 365 e qualsiasi fonte IMAP/SMTP o CalDAV.',
          items: ['Controlli SOC 2 e ISO', 'Permessi granulari', 'Log per audit completi'],
        },
      ],
      testimonial: {
        quote:
          'MailAgent ha permesso al nostro supporto di 70 persone di lavorare come se fossimo in 200. L’assistente AI scrive bozze perfette mentre il cruscotto evidenzia dove serve l’intervento umano.',
        author: 'Chiara Leone',
        role: 'Head of Service Operations, Aeris',
      },
      secondaryCta: {
        title: 'Pronti a ridisegnare le vostre email?',
        subtitle: 'Porta ogni flusso di lavoro in un workspace AI-native con MailAgent.',
        primaryCta: 'Parla con il prodotto',
        secondaryCta: 'Scarica la scheda',
      },
    },
    auth: {
      login: {
        title: 'Bentornato',
        subtitle: 'Accesso protetto con MFA e controlli di sessione.',
        form: {
          emailLabel: 'Email aziendale',
          passwordLabel: 'Password',
          submit: 'Accedi',
          cta: 'Ti serve un account?',
        },
        links: {
          primary: { href: '/auth/register', label: 'Crea un account' },
          secondary: { href: '/auth/forgot-password', label: 'Password dimenticata?' },
        },
      },
      otp: {
        title: 'Verifica il codice',
        subtitle: 'Controlla la tua casella per il codice di sicurezza a 6 cifre.',
        form: {
          otpLabel: 'Codice temporaneo',
          submit: 'Verifica e continua',
        },
        links: {
          primary: { href: '/auth/login', label: 'Torna al login' },
        },
      },
      register: {
        title: 'Crea il tuo workspace',
        subtitle: 'Configura la prima inbox in meno di due minuti.',
        form: {
          firstNameLabel: 'Nome',
          firstNamePlaceholder: 'Mario',
          lastNameLabel: 'Cognome',
          lastNamePlaceholder: 'Rossi',
          emailLabel: 'Email aziendale',
          passwordLabel: 'Password',
          confirmPasswordLabel: 'Conferma password',
          submit: 'Crea account',
          agreeToTerms: 'Accetto i ',
          termsOfService: 'Termini di Servizio',
          and: ' e la ',
          privacyPolicy: 'Privacy Policy',
          termsRequired: 'Devi accettare i Termini di Servizio e la Privacy Policy per registrarti.',
        },
        links: {
          primary: { href: '/auth/login', label: 'Hai già un account? Accedi' },
        },
      },
      forgotPassword: {
        title: 'Reimposta la password',
        subtitle: 'Ti invieremo un link sicuro via email.',
        form: {
          emailLabel: 'Email aziendale',
          submit: 'Invia link di reset',
        },
        success: {
          title: 'Controlla la posta',
          description: 'Se l’account esiste abbiamo spedito un link di ripristino.',
          actionLabel: 'Torna al login',
        },
      },
      resetPassword: {
        title: 'Imposta una nuova password',
        subtitle: 'La password deve contenere almeno 12 caratteri.',
        form: {
          passwordLabel: 'Nuova password',
          confirmPasswordLabel: 'Conferma password',
          submit: 'Aggiorna password',
        },
        success: {
          title: 'Password aggiornata',
          description: 'Ora puoi accedere con le nuove credenziali.',
          actionLabel: 'Vai al login',
        },
      },
    },
    dashboard: {
      index: {
        loading: 'Caricamento...',
        redirecting: 'Reindirizzamento alla posta...',
      },
      home: {
        stats: {
          unreadEmails: 'Email non lette',
          todayEvents: "Eventi di oggi",
          pendingTasks: 'Attività in sospeso',
          contacts: 'Contatti',
        },
        sections: {
          upcomingEvents: 'Prossimi Eventi',
          upcomingEventsSubtitle: 'La tua agenda di oggi',
          noEvents: 'Nessun evento programmato per oggi',
          viewCalendar: 'Vedi Calendario',
          priorityInbox: 'Posta Prioritaria',
          priorityInboxSubtitle: 'Email stellate e importanti',
          noStarredEmails: 'Nessuna email stellata',
          viewAllEmails: 'Vedi Tutte le Email',
          aiInsights: 'Insights AI',
          aiInsightsSubtitle: 'Suggerimenti intelligenti per te',
          smartReplyReady: 'Smart Reply Pronta',
          aiAssistMessage: 'Hai {count} email non lette che possono essere gestite con l\'assistenza AI.',
          aiAssistMessageSingular: 'Hai {count} email non letta che può essere gestita con l\'assistenza AI.',
          viewSuggestions: 'Vedi Suggerimenti',
          inboxStatus: 'Stato Inbox',
          inboxStatusMessage: '{count} non lette - Continua così!',
          quickActions: 'Azioni Rapide',
          analyzeInbox: 'Analizza Inbox',
          smartCompose: 'Smart Compose',
          recentConnections: 'Connessioni Recenti',
          recentConnectionsSubtitle: 'Ultimi contatti',
          noRecentContacts: 'Nessun contatto recente',
        },
      },
      email: {
        title: 'Control room della posta',
        descriptionTemplate: '{count} conversazioni non lette — concentrati su ciò che conta.',
        searchPlaceholder: 'Cerca conversazioni, mittenti o etichette',
        noProviders: 'Collega un provider per inviare e ricevere email.',
        selectEmail: 'Seleziona una conversazione per iniziare a collaborare.',
        allAccountsLabel: 'Tutti gli account',
        starredFolderLabel: 'Speciali',
        quickFilters: {
          title: 'Filtri Rapidi',
          unread: 'Non lette',
          today: 'Oggi',
          thisWeek: 'Questa settimana',
          hasAttachments: 'Con allegati',
          important: 'Importanti',
        },
        bulkBar: {
          selected: '{count} elementi',
          markUnread: 'Segna come non letto',
          markRead: 'Segna come letto',
          delete: 'Elimina',
          archive: 'Archivia',
          selectAll: 'Seleziona tutto',
          clear: 'Cancella selezione',
          moreActions: 'Altre azioni',
          addStar: 'Aggiungi Stella',
          removeStar: 'Rimuovi Stella',
          addLabels: 'Aggiungi Etichette',
          moveToFolder: 'Sposta nella Cartella',
        },
        messages: {
          loadMoreFailed: 'Impossibile caricare altre email',
          markRead: '{count} email segnate come lette',
          markReadFailed: 'Impossibile segnare le email come lette',
          markUnread: '{count} email segnate come non lette',
          markUnreadFailed: 'Impossibile segnare le email come non lette',
          deleteConfirm: 'Sei sicuro di voler eliminare {count} email?',
          deleted: '{count} email eliminate',
          deleteFailed: 'Impossibile eliminare le email',
          starred: '{count} email contrassegnate con stella',
          starFailed: 'Impossibile aggiungere la stella alle email',
          unstarred: '{count} email senza stella',
          unstarFailed: 'Impossibile rimuovere la stella dalle email',
          labelsAdded: 'Etichette aggiunte a {count} email',
          labelsFailed: 'Impossibile aggiungere le etichette',
          moved: '{count} email spostate in {folder}',
          moveFailed: 'Impossibile spostare le email',
          loadLabelsFailed: 'Impossibile caricare le etichette',
          selectedCount: '{count} selezionate',
          move: 'Sposta',
          emailSent: 'Email inviata con successo ✓',
        },
        composer: {
          title: 'Nuovo Messaggio',
          titleReply: 'Rispondi',
          titleForward: 'Inoltra',
          from: 'Da',
          to: 'A',
          toLabel: 'A:',
          toPlaceholder: 'destinatario@example.com',
          cc: 'Cc',
          ccLabel: 'Cc:',
          bcc: 'Ccn',
          bccLabel: 'Ccn:',
          subject: 'Oggetto',
          subjectLabel: 'Oggetto:',
          subjectPlaceholder: 'Oggetto email',
          bodyLabel: 'Messaggio',
          bodyPlaceholder: 'Componi il tuo messaggio...',
          attachFiles: 'Allega file',
          removeAttachment: 'Rimuovi allegato',
          attachments: 'Allegati ({count})',
          formatting: 'Formattazione',
          undo: 'Annulla',
          redo: 'Ripeti',
          heading1: 'Titolo 1',
          heading2: 'Titolo 2',
          heading3: 'Titolo 3',
          bold: 'Grassetto',
          italic: 'Corsivo',
          underline: 'Sottolineato',
          strikethrough: 'Barrato',
          bulletList: 'Elenco puntato',
          numberedList: 'Elenco numerato',
          quote: 'Citazione',
          link: 'Inserisci link',
          send: 'Invia',
          saveDraft: 'Salva bozza',
          discard: 'Scarta',
          sending: 'Invio...',
          saving: 'Salvataggio...',
          savedJustNow: 'Salvato proprio ora',
          savedMinutesAgo: 'Salvato {minutes}m fa',
          savedHoursAgo: 'Salvato {hours}h fa',
          selectProvider: 'Seleziona account email',
          noProvider: 'Nessun provider email selezionato',
          validationTo: 'Inserisci almeno un destinatario',
          validationSubject: 'Inserisci un oggetto',
          sendError: 'Invio fallito. Riprova.',
          uploadError: 'Impossibile caricare gli allegati',
          fileTooLarge: 'File {filename} troppo grande (max 10MB)',
          draftSaved: 'Bozza salvata con successo',
          discardConfirm: 'Scartare le modifiche non salvate?',
        },
        advancedSearch: {
          title: 'Ricerca Avanzata',
          active: 'attivi',
          searchInSubjectBody: 'Cerca nell\'oggetto e nel corpo',
          enterKeywords: 'Inserisci parole chiave...',
          fromSender: 'Da (Mittente)',
          senderPlaceholder: 'mittente@example.com',
          filterBySender: 'Filtra per indirizzo email del mittente',
          dateRange: 'Intervallo Date',
          fromDate: 'Data Inizio',
          toDate: 'Data Fine',
          statusFilters: 'Filtri Stato',
          hasAttachments: 'Ha allegati',
          starredOnly: 'Solo con stella',
          readStatus: 'Stato Lettura',
          unreadOnly: 'Solo non letti',
          readOnly: 'Solo letti',
          resetAll: 'Ripristina Tutto',
          cancel: 'Annulla',
          search: 'Cerca',
        },
        attachment: 'Allegato',
      },
      labels: {
        title: 'Etichette',
        manageLabels: 'Gestisci Etichette',
        addLabel: 'Aggiungi Etichetta',
        createLabel: 'Crea Etichetta',
        editLabel: 'Modifica Etichetta',
        labelName: 'Nome Etichetta',
        labelNameRequired: "Il nome dell'etichetta è obbligatorio",
        chooseColor: 'Scegli un colore',
        createButton: 'Crea',
        updateButton: 'Aggiorna',
        cancelButton: 'Annulla',
        deleteConfirm: 'Sei sicuro di voler eliminare questa etichetta?',
        failedToCreate: "Impossibile creare l'etichetta",
        failedToUpdate: "Impossibile aggiornare l'etichetta",
        failedToDelete: "Impossibile eliminare l'etichetta",
        noLabels: 'Nessuna etichetta',
        noLabelsDescription: 'Crea etichette per organizzare le tue email',
        closeButton: 'Chiudi',
        manageLabelsTooltip: 'Gestisci Etichette',
        addRemoveLabels: 'Aggiungi o rimuovi etichette',
        noLabelsAvailable: 'Nessuna etichetta disponibile',
        addLabelText: 'Aggiungi etichetta',
      },
      conversations: {
        title: 'Conversazioni',
        noConversations: 'Nessuna conversazione trovata',
        noConversationsDescription: 'Prova a modificare i filtri o inizia una nuova email',
        failedToLoad: 'Impossibile caricare le conversazioni',
        loadMore: 'Carica Altri',
        loading: 'Caricamento...',
        messages: 'messaggi',
        participants: 'partecipanti',
        yesterday: 'Ieri',
        retry: 'Riprova',
      },
      emailList: {
        loading: 'Caricamento email...',
        empty: 'Nessuna email corrisponde ai filtri.',
        threads: { singular: 'thread', plural: 'thread' },
        inboxLabel: 'Posta in arrivo',
        advancedSearch: 'Ricerca Avanzata',
        emailListView: 'Elenco Email',
        conversationView: 'Vista Conversazione',
        openSidebar: 'Apri barra laterale',
        markAsRead: 'Segna come letto',
        markAsUnread: 'Segna come non letto',
        archive: 'Archivia',
      },
      emailView: {
        folderLabel: 'Cartella',
        attachments: 'Allegati',
        to: 'A',
        cc: 'Cc',
        labels: 'Etichette',
        date: 'Data',
        selectEmail: 'Seleziona un\'email per vedere l\'intera conversazione.',
        back: 'Indietro',
        archive: 'Archivia',
        more: 'Altro',
        download: 'Scarica',
        reply: 'Rispondi',
        replyAll: 'Rispondi a tutti',
        forward: 'Inoltra',
        summaryTitle: 'Riassunto AI',
        summaryGenerate: 'Genera riassunto',
        summaryRegenerate: 'Rigenera riassunto',
        summaryShow: 'Mostra Riassunto',
        summaryHide: 'Nascondi Riassunto',
        summaryEmpty: 'Chiedi al Copilot di spiegare questa conversazione.',
        categoriesGenerate: 'Categorizza con AI',
        categoriesLoaded: 'Categorie caricate',
        categoriesLoading: 'Categorizzazione...',
        smartRepliesTitle: 'Risposte suggerite',
        smartRepliesGenerate: 'Genera risposte suggerite',
        smartRepliesRegenerate: 'Rigenera risposte',
        smartRepliesLoaded: 'Risposte caricate',
        smartRepliesLoading: 'Recupero delle risposte suggerite...',
        smartRepliesEmpty: 'Nessuna risposta suggerita al momento.',
        labelTitle: 'Etichette suggerite',
        labelEmpty: 'Qui appariranno le etichette proposte dall\'AI.',
        memoryTitle: 'Memoria del workspace',
        memoryDescription: 'Consulta il livello RAG per recuperare conversazioni simili, documenti e decisioni passate.',
        memoryPlaceholder: 'Aggiungi parole chiave per affinare la ricerca (facoltativo)',
        memoryGenerate: 'Cerca nella memoria',
        memoryRegenerate: 'Aggiorna memoria',
        memoryLoading: 'Recupero dei ricordi più rilevanti...',
        memoryEmpty: 'Nessun ricordo trovato. Prova con un termine diverso.',
        memoryCopy: 'Copia',
        memoryCopied: 'Copiato',
        memoryUse: 'Usa nella risposta',
        memoryQueryRequired: 'Inserisci una ricerca o apri un\'email per usare la memoria.',
        memorySourceEmail: 'Email',
        memorySourceDocument: 'Documento',
        memoryConfidenceLabel: 'Forza corrispondenza',
        memoryConfidenceHigh: 'Alta',
        memoryConfidenceMedium: 'Media',
        memoryConfidenceLow: 'Bassa',
        memoryUnknownSender: 'Mittente sconosciuto',
        memoryLastQueryPrefix: 'Ricerca usata:',
        memoryCopyError: 'Impossibile copiare questo elemento ora.',
        memoryError: 'Impossibile interrogare la memoria al momento.',
      },
      calendar: {
        title: 'Calendario',
        description: 'Gestisci i tuoi eventi del calendario',
        refreshTooltip: 'Aggiorna calendari',
        allCalendars: 'Tutti i Calendari',
        clearSelection: 'Nascondi tutto',
        filterTitle: 'Calendari',
        noProviders: 'Nessun provider calendario configurato',
        noProvidersDescription: 'Configura un provider calendario nella pagina Provider',
        loading: 'Caricamento calendario...',
        createEvent: 'Nuovo Evento',
        editEvent: 'Modifica Evento',
        deleteEvent: 'Elimina Evento',
        deleteConfirm: 'Sei sicuro di voler eliminare questo evento?',
        eventDetails: 'Dettagli Evento',
        startTime: 'Ora Inizio',
        endTime: 'Ora Fine',
        location: 'Luogo',
        eventDescription: 'Descrizione',
        attendees: 'Partecipanti',
        saveEvent: 'Salva Evento',
        cancelEvent: 'Annulla',
        quickEventPlaceholder: 'Pianifica una riunione del team per venerdì mattina...',
        today: 'Oggi',
        day: 'Giorno',
        week: 'Settimana',
        month: 'Mese',
        eventTitle: 'Titolo Evento',
        eventTitleLabel: 'Titolo Evento',
        startDateTime: 'Data/Ora Inizio',
        endDateTime: 'Data/Ora Fine',
        calendarLabel: 'Calendario',
        noProviderAlert: 'Nessun provider calendario disponibile. Collega prima un provider calendario.',
        createEventDialogTitle: 'Crea Nuovo Evento',
        aiInsightsTitle: 'Suggerimenti AI',
        aiOptimizeMessage: 'Hai {count} eventi programmati. Vuoi che l\'AI ottimizzi il tuo calendario?',
        openAiAssistant: 'Apri Assistente AI',
        noEventsMessage: 'Nessun evento programmato. Clicca sul calendario per creare il tuo primo evento!',
        failedToCreateEvent: 'Impossibile creare l\'evento. Riprova.',
        failedToCreateQuickEvent: 'Impossibile creare l\'evento rapido. Riprova.',
      },
      contacts: {
        title: 'Contatti',
        description: 'Rubrica unificata per tutti i provider collegati.',
        searchPlaceholder: 'Cerca contatti...',
        companyPlaceholder: 'Azienda',
        providerFilterLabel: 'Provider',
        allProvidersLabel: 'Tutti i provider',
        applyFilters: 'Applica filtri',
        clearFilters: 'Pulisci filtri',
        refresh: 'Aggiorna',
        sync: 'Sincronizza provider',
        syncing: 'Sincronizzazione in corso...',
        syncDisabledTooltip: 'Seleziona un provider da sincronizzare',
        new: 'Nuovo',
        noContactsFound: 'Nessun contatto trovato',
        selectContactMessage: 'Seleziona un contatto per visualizzare i dettagli',
        actions: {
          email: 'Email',
          schedule: 'Programma',
          addNote: 'Aggiungi Nota',
          aiInsights: 'Suggerimenti AI',
        },
        details: {
          contactInformation: 'Informazioni Contatto',
          email: 'Email',
          phone: 'Telefono',
          company: 'Azienda',
          location: 'Posizione',
          lastInteraction: 'Ultima Interazione',
          notAvailable: 'Non disponibile',
        },
        tabs: {
          details: 'Dettagli',
          activity: 'Attività',
          notes: 'Note',
        },
        activity: {
          title: 'Attività Recente',
          noActivity: 'Nessuna attività recente',
        },
        notes: {
          title: 'Note',
          noNotes: 'Nessuna nota ancora',
          addNote: 'Aggiungi Nota',
        },
        list: {
          emptyTitle: 'Nessun contatto disponibile',
          emptyDescription: 'Collega un provider con contatti o modifica i filtri.',
          loading: 'Caricamento contatti...',
          showing: 'Visualizzati {count} di {total} contatti',
          tableHeaders: {
            name: 'Nome',
            email: 'Email',
            phone: 'Telefono',
            company: 'Azienda',
            provider: 'Provider',
            lastSynced: 'Ultima sincronizzazione',
            actions: 'Azioni',
          },
        },
        alerts: {
          loadError: 'Impossibile caricare i contatti. Riprova più tardi.',
          syncSuccess: 'Sincronizzazione contatti completata con successo.',
          syncError: 'Impossibile sincronizzare i contatti. Riprova.',
          deleteConfirmTitle: 'Elimina contatto',
          deleteConfirmDescription: 'Sei sicuro di voler eliminare questo contatto?',
          deleteSuccess: 'Contatto eliminato con successo.',
          deleteError: 'Impossibile eliminare questo contatto. Riprova.',
          selectProvider: 'Seleziona un provider',
          failedToSave: 'Impossibile salvare il contatto. Riprova.',
        },
        createDialog: {
          title: 'Nuovo Contatto',
          provider: 'Provider',
          name: 'Nome',
          email: 'Email',
          company: 'Azienda',
          jobTitle: 'Titolo Professionale',
          notes: 'Note',
          create: 'Crea Contatto',
          cancel: 'Annulla',
        },
        editDialog: {
          title: 'Modifica Contatto',
          description: 'Aggiorna le informazioni di base per questo contatto.',
          firstName: 'Nome',
          lastName: 'Cognome',
          displayName: 'Nome visualizzato',
          name: 'Nome',
          email: 'Email',
          phone: 'Telefono principale',
          company: 'Azienda',
          jobTitle: 'Titolo Professionale',
          notes: 'Note',
          save: 'Salva Modifiche',
          cancel: 'Annulla',
          success: 'Contatto aggiornato con successo.',
          error: 'Impossibile aggiornare questo contatto. Riprova.',
        },
      },
      tasks: {
        title: 'Attività',
        workspace: 'Area di Lavoro',
        allTasks: 'Tutte le Attività',
        today: 'Oggi',
        upcoming: 'Prossime',
        myProjects: 'I Miei Progetti',
        addNewList: 'Aggiungi Nuova Lista',
        filter: 'Filtra',
        newTask: 'Nuova Attività',
        addTaskPlaceholder: 'Aggiungi un\'attività per Oggi, premi Invio per salvare',
        taskDetails: 'Dettagli Attività',
      },
      providers: {
        title: 'Provider e Integrazioni',
        descriptionTemplate: 'Collega caselle di posta, calendari e origini sincronizzate per {email}',
        refresh: 'Aggiorna',
        refreshing: 'Aggiornamento...',
        connectedTab: 'Provider Collegati',
        addTab: 'Aggiungi Provider',
        connectedTitle: 'Provider collegati',
        connectedDescription: 'Gestisci inbox, calendari e fonti di automazione collegate per il tuo tenant.',
        loading: 'Caricamento provider...',
        googleTitle: 'Google',
        googleDescription: 'Collega Gmail e Google Calendar',
        googleConnect: 'Collega Account Google',
        googleConnecting: 'Connessione...',
        googleFeatures: {
          description: 'Collega il tuo account Google per sincronizzare email ed eventi del calendario.',
          feature1: 'Leggi e invia email tramite Gmail',
          feature2: 'Accedi agli eventi di Google Calendar',
          feature3: 'Sincronizza contatti (opzionale)',
        },
        microsoftTitle: 'Microsoft',
        microsoftDescription: 'Collega Outlook e Microsoft Calendar',
        microsoftConnect: 'Collega Account Microsoft',
        microsoftConnecting: 'Connessione...',
        microsoftFeatures: {
          description: 'Collega il tuo account Microsoft per accedere a email e calendario.',
          feature1: 'Leggi e invia email tramite Outlook',
          feature2: 'Accedi agli eventi di Microsoft Calendar',
          feature3: 'Sincronizza contatti (opzionale)',
        },
        genericTitle: 'IMAP/CalDAV Generico',
        genericDescription: 'Collega qualsiasi provider IMAP/SMTP o CalDAV',
        genericConnect: 'Configura Provider Generico',
        genericFeatures: {
          description: 'Collega qualsiasi provider IMAP/SMTP o CalDAV per unificare email e calendario.',
          feature1: 'Accesso email IMAP/SMTP',
          feature2: 'Sincronizzazione calendario CalDAV',
          feature3: 'Sincronizzazione contatti CardDAV',
        },
        genericCard: {
          title: 'IMAP & CalDAV',
          description: 'Collega provider IMAP/SMTP e CalDAV personalizzati',
          subtitle: 'Porta qualsiasi casella di posta o calendario compatibile inserendo le credenziali del server.',
          feature1: 'Supporto IMAP/SMTP per email in entrata/uscita',
          feature2: 'Sincronizzazione CalDAV opzionale per calendari',
          feature3: 'Perfetto per Fastmail, Yahoo, iCloud e server personalizzati',
          configureButton: 'Configura IMAP / CalDAV',
          dialogTitle: 'Collega Provider IMAP/SMTP & CalDAV',
          dialogSubtitle: 'Configura le impostazioni del server email e calendario',
          basicInfo: 'Informazioni Base',
          emailAddress: 'Indirizzo Email',
          displayName: 'Nome Visualizzato',
          imapSettings: 'Impostazioni IMAP (Posta in Arrivo)',
          imapHost: 'Host IMAP',
          imapHostPlaceholder: 'imap.esempio.com',
          port: 'Porta',
          username: 'Nome Utente',
          password: 'Password',
          smtpSettings: 'Impostazioni SMTP (Posta in Uscita)',
          smtpSubtitle: 'Lascia vuoto per usare le credenziali IMAP',
          smtpHost: 'Host SMTP',
          smtpHostPlaceholder: 'smtp.esempio.com',
          caldavSettings: 'Impostazioni CalDAV (Sincronizzazione Calendario)',
          enableCalendar: 'Abilita Calendario',
          caldavUrl: 'URL CalDAV',
          caldavUrlPlaceholder: 'https://caldav.esempio.com/calendars/utente',
          cancel: 'Annulla',
          connecting: 'Connessione...',
          connectProvider: 'Collega Provider',
        },
        helpTitle: 'Serve Aiuto?',
        helpGoogle: 'Clicca "Collega Account Google" e accedi con le tue credenziali Google. Concedi l\'accesso a Gmail, Calendar e Contatti.',
        helpMicrosoft: 'Usa "Collega Account Microsoft" e accedi con le tue credenziali Outlook/Microsoft 365.',
        helpGeneric: 'Fornisci i dettagli del server IMAP/SMTP dal tuo provider. CalDAV abilita la sincronizzazione del calendario.',
        commonSettingsTitle: 'Impostazioni IMAP/SMTP Comuni:',
        successMessage: 'Provider collegato con successo!',
        deleteSuccess: 'Provider disconnesso con successo!',
        deleteConfirm: 'Sei sicuro di voler disconnettere {email}?',
        disconnecting: 'Disconnessione...',
        disconnect: 'Disconnetti',
        oauthError: 'Errore OAuth:',
        addProvider: 'Aggiungi un provider',
        noProvidersMessage: 'Nessun provider collegato ancora. Collega un provider per iniziare.',
        badges: {
          default: 'Predefinito',
          active: 'Attivo',
          inactive: 'Inattivo',
          email: 'Email',
          calendar: 'Calendario',
          contacts: 'Contatti',
        },
        lastSynced: 'Ultima sincronizzazione: {date}',
        neverSynced: 'Mai',
      },
      settings: {
        title: 'Impostazioni Workspace',
        description: 'Aggiorna il tuo profilo personale, gestisci i provider collegati e gestisci le azioni dell\'account.',
        preferences: 'Preferenze',
        profileTitle: 'Impostazioni Profilo',
        profileDescription: 'Mantieni aggiornate le tue informazioni personali.',
        firstName: 'Nome',
        lastName: 'Cognome',
        emailReadonly: 'Email (sola lettura)',
        saving: 'Salvataggio...',
        saveChanges: 'Salva modifiche',
        successMessage: 'Impostazioni aggiornate con successo',
        providersTitle: 'Provider Email e Calendario',
        providersDescription: 'Gestisci gli account che alimentano la tua inbox e la sincronizzazione del calendario.',
        manageProviders: 'Gestisci Provider',
        dangerZoneTitle: 'Zona Pericolosa',
        dangerZoneDescription: 'Elimina il tuo account e tutti i dati associati in modo permanente.',
        dangerZoneWarning: 'Questa azione non può essere annullata. Assicurati di aver esportato tutte le informazioni importanti prima di procedere.',
        deleteAccount: 'Elimina account',
        deleting: 'Eliminazione...',
        deleteConfirm: 'Sei sicuro di voler eliminare il tuo account? Questa azione non può essere annullata.',
        sidebarTitle: 'Impostazioni',
        sidebarSubtitle: 'Gestisci le tue preferenze',
        sections: {
          general: 'Generale',
          ai: 'Agente AI',
          accounts: 'Account email',
          account: 'Account',
          notifications: 'Notifiche',
        },
        generalPanel: {
          title: 'Impostazioni generali',
          description: 'Configura aspetto, lingua e notifiche di base.',
          appearanceTitle: 'Aspetto',
          themeLabel: 'Tema',
          themeHint: 'Scegli "Sistema" per adattarti automaticamente alle impostazioni del dispositivo.',
          themeOptions: {
            light: 'Chiaro',
            dark: 'Scuro',
            system: 'Sistema',
          },
          languageTitle: 'Lingua e regione',
          languageLabel: 'Lingua',
          languageHint: 'Questa opzione riguarda solo l\'interfaccia di MailAgent.',
          languageOptions: {
            en: 'English (US)',
            it: 'Italiano',
            es: 'Español',
            fr: 'Français',
            de: 'Deutsch',
          },
          timezoneLabel: 'Fuso orario',
          timezoneOptions: {
            pst: '(GMT-08:00) Ora del Pacifico',
            est: '(GMT-05:00) Ora Orientale',
            gmt: '(GMT+00:00) Londra',
            cet: '(GMT+01:00) Roma',
            jst: '(GMT+09:00) Tokyo',
          },
          notificationsTitle: 'Notifiche',
          notificationsDescription: 'Personalizza il modo in cui ricevi gli avvisi.',
          notificationOptions: {
            newEmails: 'Nuove Email',
            newEmailsDescription: 'Ricevi notifiche quando arrivano nuove email',
            calendarReminders: 'Promemoria Calendario',
            calendarRemindersDescription: 'Promemoria per eventi imminenti',
            taskDeadlines: 'Scadenze Attività',
            taskDeadlinesDescription: 'Avvisi quando le attività sono in scadenza',
          },
          reset: 'Reimposta',
          save: 'Salva modifiche',
        },
        mailAccountsPanel: {
          title: 'Account email',
          description: 'Riepilogo degli account che alimentano posta, calendario e contatti.',
          empty: 'Nessun account collegato. Usa i pulsanti qui sotto per aggiungerne uno.',
          manage: 'Gestisci provider',
          add: 'Aggiungi provider',
          refresh: 'Aggiorna',
          defaultBadge: 'Predefinito',
          emailBadge: 'Email',
          calendarBadge: 'Calendario',
          contactsBadge: 'Contatti',
          lastSyncPrefix: 'Ultima sincronizzazione:',
          neverSynced: 'Mai sincronizzato',
          connectedProviders: 'Provider collegati',
          connectedProvidersSubtitle: 'Gestisci inbox collegate, calendari e fonti di automazione per il tuo tenant.',
          providerTypes: {
            google: 'Google Workspace',
            microsoft: 'Microsoft 365',
            generic: 'IMAP/SMTP generico',
          },
          loadError: 'Impossibile caricare i provider. Riprova.',
          connectSuccess: 'Provider collegato con successo!',
          connectRedirect: 'Reindirizzamento al provider. Completa il flusso di autorizzazione per terminare la connessione.',
          disconnectSuccess: 'Provider scollegato con successo!',
          saveSuccess: 'Impostazioni salvate con successo',
          saveError: 'Impossibile salvare le impostazioni',
          helpTitle: 'Serve Aiuto?',
          helpGoogle: 'Google: Clicca "Connetti Account Google" e accedi con le tue credenziali Google. Assicurati di avere accesso a Gmail e Google Calendar.',
          helpMicrosoft: 'Microsoft: Usa "Connetti Account Microsoft" e accedi con le tue credenziali Microsoft 365 o Outlook.com.',
          helpIMAP: 'IMAP Generico: Fornisci i dettagli del server IMAP/SMTP, il tuo indirizzo email e la password o password specifica per l\'app.',
          helpCommonSettings: 'Impostazioni IMAP/SMTP comuni:',
          helpGmail: 'Gmail: imap.gmail.com:993, smtp.gmail.com:587 (richiede password app)',
          helpOutlook: 'Outlook.com: outlook.office365.com:993, smtp.office365.com:587',
          helpYahoo: 'Yahoo: imap.mail.yahoo.com:993, smtp.mail.yahoo.com:587',
        },
        aiPanel: {
          title: 'Impostazioni agente AI',
          description: 'Definisci come il copilota ti supporta nel workspace.',
          smartReplies: 'Risposte intelligenti',
          smartRepliesDescription: 'Suggerimenti AI per risposte rapide.',
          summarization: 'Riepilogo email',
          summarizationDescription: 'Contesto automatico per thread lunghi.',
          scheduling: 'Pianificazione smart',
          schedulingDescription: 'Suggerisci automaticamente orari per riunioni.',
          save: 'Salva modifiche',
        },
        accountPanel: {
          title: 'Impostazioni account',
          description: 'Gestisci le informazioni del profilo.',
          profileInformation: 'Informazioni profilo',
          fullName: 'Nome completo',
          email: 'Email',
          role: 'Ruolo',
          readOnlyNotice: 'I dettagli dell’account sono gestiti centralmente. Contatta l’amministratore per eventuali modifiche.',
          dangerTitle: 'Zona pericolosa',
          dangerDescription: 'Elimina il tuo account e tutti i dati associati in modo permanente.',
          deleteCta: 'Elimina account',
        },
        notificationsPanel: {
          title: 'Impostazioni notifiche',
          description: 'Controlla come ricevi gli avvisi.',
          emailToggle: 'Notifiche email',
        },
      },
      folders: {
        all: 'Tutta la Posta',
        inbox: 'Posta in Arrivo',
        sent: 'Inviata',
        drafts: 'Bozze',
        trash: 'Cestino',
        spam: 'Posta indesiderata',
        archive: 'Archivio',
        outbox: 'Posta in uscita',
        starred: 'Speciali',
      },
    },
    cookies: {
      title: 'Impostazioni Cookie',
      description: 'Utilizziamo i cookie per migliorare la tua esperienza di navigazione, fornire contenuti personalizzati e analizzare il nostro traffico. Cliccando "Accetta Tutti", acconsenti all\'uso dei nostri cookie.',
      privacyLink: 'Privacy Policy',
      customize: 'Personalizza',
      showDetails: 'Mostra dettagli',
      hideDetails: 'Nascondi dettagli',
      necessary: {
        title: 'Cookie Necessari',
        description: 'Essenziali per il corretto funzionamento del sito. Non possono essere disattivati.',
      },
      analytics: {
        title: 'Cookie Analitici',
        description: 'Ci aiutano a capire come i visitatori interagiscono con il nostro sito.',
      },
      marketing: {
        title: 'Cookie Marketing',
        description: 'Utilizzati per fornire pubblicità personalizzate.',
      },
      acceptAll: 'Accetta Tutti',
      rejectAll: 'Rifiuta Tutti',
      savePreferences: 'Salva Preferenze',
    },
    errors: {
      notFound: {
        title: '404',
        subtitle: 'Pagina Non Trovata',
        description: 'Siamo spiacenti, non riusciamo a trovare la pagina che stai cercando.',
        action: 'Torna alla Home',
      },
      serverError: {
        title: '500',
        subtitle: 'Errore del Server',
        description: 'Qualcosa è andato storto da parte nostra. Riprova più tardi.',
        action: 'Torna alla Home',
      },
    },
  },
};


